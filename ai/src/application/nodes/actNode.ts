 ...
 * walletAddress comes from state.strategy — each user trades from their own wallet.
 */
    ...
    for (const decision of state.validatedDecisions) {
      try {
        ...
        if (decision.action === "trade" || decision.action === "close_position") {
          ...
          result = { success: true, dryRun: strategy.dry_run };
        }
        ...
        await auditLogger.writeLog({
          ...
          data: { user: strategy.ensName, decision, result },
        });
        results.push(result);
      } catch (err) {
        results.push({
